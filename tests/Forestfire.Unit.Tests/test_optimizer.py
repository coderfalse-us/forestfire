...
from src.utils.config import WarehouseConfigManager, WarehouseConfig
...
class TestWarehouseOptimizer:
    ...
    async def test_optimize_main(self, optimizer, test_warehouse_config):
        ...
        # Create mock data for return values
        test_data = (
        ...
        with (
            ...
            patch.object(optimizer.path_visualizer, "plot_routes") as mock_plot,
            patch.object(
                optimizer.batch_pick_sequence_service, "update_pick_sequences"
            ) as mock_update,
        ):
            # Create an async function that returns our test data
            async def mock_get_data_func(*args, **kwargs):
                return test_data

            # Create async functions that do nothing (for methods that don't return values)
            async def mock_plot_func(*args, **kwargs):
                return None

            async def mock_update_func(*args, **kwargs):
                return None

            # Assign the async functions as side_effects
            mock_get_data.side_effect = mock_get_data_func
            mock_plot.side_effect = mock_plot_func
            mock_update.side_effect = mock_update_func

            # Execute with config_manager
