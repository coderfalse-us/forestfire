...
class TestAntColonyOptimizer:
    ...
    def test_calculate_heuristic(self, ant_colony_optimizer, sample_orders_assign):
        ...
        assert heuristic.shape == (
            len(sample_orders_assign), len(picker_locations))
        assert np.all(heuristic > 0)  # Heuristic values should be positive

    def test_build_solution(self, ant_colony_optimizer, sample_pheromone,
                           sample_heuristic):
    ...
    def test_update_pheromone(self, ant_colony_optimizer, sample_pheromone):
        ...
        for item in range(orders_size):
            ...
            assert (sample_pheromone[item][picker] !=
    ...
    def test_build_solution_with_full_pickers(self, ant_colony_optimizer,
                                           sample_pheromone, sample_heuristic):
        ...
        assert all(picker_id == -1 or (0 <= picker_id < NUM_PICKERS)
